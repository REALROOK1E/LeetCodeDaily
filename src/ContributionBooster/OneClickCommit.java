...
public class OneClickCommit {
    ...
    private static String createJavaFile(String code) throws IOException {
        String fileName = "src/ContributionBooster/" + FUNCTION_NAMES[random.nextInt(FUNCTION_NAMES.length)] + ".java";
        
        // 确保目录存在
        File directory = new File("src/ContributionBooster");
        if (!directory.exists()) {
            directory.mkdirs();
        }
    ...
    private static void updateExistingFiles() throws IOException {
        ...
        String[] filesToUpdate = {
            "src/ContributionBooster/DailyCommit.java",
            "src/ContributionBooster/RandomCode.java",
            "src/ContributionBooster/CommitHelper.java"
        };
